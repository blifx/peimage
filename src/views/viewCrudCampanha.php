...
<html lang="pt-br" >
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                $("#campanhas").change(function(){
                    $("#inicio").val($("#campanhas option:selected").attr("dt_inicio"));
                    $("#encerramento").val($("#campanhas option:selected").attr("dt_encerramento"));
                });

                $("#loadingSubmit").click(function(){
                    alert("Aguarde mais um pouco, seus arquivos estão sendo processados.");
                });

                $("#update").click(function(){

                    //restringimos campanhas com data iniciais maior que o dia atual
                    var dt1 = new Date($("#inicio").val() + " 00:00");
                    var dt2 = new Date();
                    ...
                    alert("Aviso:\nAo inserir um novo tema esse layout será inativado e deverá ser reconfigurado na tela inicial.");

                    let el = $("#uploads-select option:selected");

                    $("#campanhas option:selected").attr("inicio");

                    myDropzone.options.url = 
                        "actionCrudCampanha.php?idcampanha="
                        + $("#campanhas option:selected").val()
                        + "&width="+el.attr("w")
                        + "&height="+el.attr("h")
                        + "&tipo="+el.attr("tipo")
                        + "&idlayout="+el.val()
                        + "&campanha="+$("#campanhas option:selected").text()
                        + "&dt_inicio="+$("#inicio").val()
                        + "&dt_encerramento="+$("#encerramento").val();
                ...
                $("#remove").click(function(){
                    if(confirm("Tem certeza que deseja excluir essa campanha?")){
                        $("input[name='act']").val("remove");
                        $("input[name='nome_campanha']").val( $("#campanhas option:selected").text() );
                        $("#waterform").submit();
                    }
                });
                ...
                $("#btn-view-file-gallery").click(function(e){
                    e.preventDefault();
                    window.open("../src/actionGaleria.php?campanha="+$("#campanhas").val()+"&act=tema", '_blank');
                });
    ...
    <body>
        ...
        <div id="content" <?php if($numCampanhas == 0) echo "class='formSmall'";?>>
        ...
        <form id="waterform" method="post" action="actionCrudCampanha.php" enctype="multipart/form-data" >
        <?php
            if(isset($_GET['msg'])){
                echo '<div id="msg-error">Campanha não pode ser inserida </br>'.$_GET['msg'].'</div>';
            }
            if(isset($_GET["response"])){
                switch($_GET["response"]){
                    case 0:
                        echo '<div id="msg-error">Houve um erro ao atualizar a campanha. </div>';
                    break;
                    case 1:
                        echo '<div id="msg-sucess">Ação realizada com sucesso. </div>';
                    break;
                }
            }
        ?>
        ...
        ?>
            ...
            <input type="hidden" name="nome_campanha" />
            <select id='campanhas' name='campanhas' required='required'>
            <?php
                while($rowsCampanhaSelect=mysqli_fetch_array($queryCampanhaSelect)){
                    echo '<option  dt_inicio="'.$rowsCampanhaSelect['dt_inicio'].'" dt_encerramento="'.$rowsCampanhaSelect['dt_encerramento'].'" value="'.$rowsCampanhaSelect['idcampanhas'].'" >'.ucfirst($rowsCampanhaSelect['nome']).'</option>';
                }
            ?>
            </select><br/>
            <label>Início</label>
            <input type="date" id="inicio" name="inicio" value="<?=$rowsCampanha['dt_inicio']?>" name="campanha" required="required" /><br/>
            <label>Encerramento</label>
            <input type="date" id="encerramento" name="encerramento" value="<?=$rowsCampanha['dt_encerramento']?>" name="campanha" required="required"/><br/>

                <label for='arquivos'>Adicionar temas</label>
                <a id="clear">Limpar</a>
                <br>
                ...
                <div style="display: flex; width: 100%; box-sizing: border-box;">
                    ...
                    <div class="btn-div" id="btn-view-file-gallery">
                        <div>Temas</div>
                    </div>

                    <div class="btn-div" style="display:none" id="loadingSubmit">
                        <div class="align"><img src="<?=$__HTTP_HOST;?>/images/rolling.gif" width="25" height="25"></div>
                        <div class="align">Carregando...</div>
                    </div>

                    <div class="btn-div" id="update">
                        <div>Atualizar</div>
                    </div>

                    <div class="btn-div" id="remove">
                        <div>Excluir</div>
                    </div>
