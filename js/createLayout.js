    ...
    function adjustScreen(){
        ...
        //com uma regra de tres nos definimos o tamanho maximo da escala baseado no tamanho disponivel do content
        //margin_image_theme adiciona uma margem extra para centralizacao da imagem
        var scaleHeight = "0." + parseInt( ( ($("#content-img-generate").height() - __MARGIN_IMAGE_THEME) * 93) / __HEIGHT);
        var scaleWidth = "0." + parseInt( ( ($("#content-img-generate").width() - __MARGIN_IMAGE_THEME) * 100) / __WIDTH);

        if($("body").width() <= 768){
            if(__HEIGHT >= __WIDTH && $("#content-img-generate").width() > __WIDTH){
                __SCALE = scaleHeight;
            } else {
                $("#content-img-generate").css("height", (__HEIGHT * parseFloat(scaleWidth)) + __MARGIN_IMAGE_THEME);
                __SCALE = scaleWidth;
            }
        } else {
            $("#content-img-generate").css("height", "calc(100% - 35px)");
            if(__HEIGHT >= __WIDTH){
                __SCALE = scaleHeight;
            } else {
                __SCALE = scaleWidth;
        ...
        var translateX = ($("#content-img-generate").width() / 2) - (__WIDTH / 2 ) + "px";
        //TODO calculo deve ser refatorado para usar translate corretamente
        var translateY = ($("#content-img-generate").height() / 2) - ( __HEIGHT / 2 ) + "px";

        $("#capture").css({
            "transform" : "scale("+ __SCALE + "," + __SCALE +") translateX("+translateX+") translateY("+translateY+")",
            //top : translateY,
            //left : translateX,
        });
    }
    ...
    function saveConfigLayout(){
        ...
        $.ajax({
            type: "POST",
            url: "../src/actionCreateLayoutA4.php",
            data: {
